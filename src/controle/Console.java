...
public class Console extends UnicastRemoteObject implements IConsole {
	 ...
	 *            graphique)
	 * @param port
	 *            numero de port de l'arene 5099 par defaut
	 * @throws RemoteException
	 */
	public Console(Element elem, int dx, int dy, int port, String ipArene)
			throws RemoteException {
		...
		try {
			...
			System.out.println("rmi://" + this.ipArene + ":" + this.port
					+ "/Arene");
			arene = (IArene) java.rmi.Naming.lookup("rmi://" + this.ipArene
					+ ":" + this.port + "/Arene");
			arene.toString();

			// initialisation de la reference du controleur sur le serveur
			// La console devient "serveur" pour les methodes de IConsole
			// lancer l'annuaire rmi en tant que serveur. A faire une seule fois
			// par serveur de console pour un port donne
			...
			Naming.rebind("rmi://localhost:" + (portServeur) + "/Console"
					+ refRMI, this);

			// initialisation de la vue sur l'element
			ve = new VueElement(refRMI, pos, this, "Atterrissage...");

			// connexion a l'arene pour lui permettre d'utiliser les methodes de
			// IConsole
			String ipConsole = InetAddress.getLocalHost()
			...
			System.out
					.println("Erreur : la console n'a pas pu etre creee !\nCause : "
							+ e.getCause());
			e.printStackTrace();
		}
	}
	...
	public void run() throws RemoteException {
		...
		if (elem instanceof Personnage) {
			...
			// applique la strategie du personnagea
	...
	public void perdreVie(int viePerdue) throws RemoteException {
		...
		elem.parler("Ouch, j'ai perdu " + viePerdue
	...
	public void changerLeader(IConsole leader) throws RemoteException {
		...
		// si existant, enlever this de l'equipe de l'ancien leader (si
		// existant)
		if (refOldLeader != -1) {
		...
		if (leader != null) {
			...
			// ajouter toute l'equipe de this a l'equipe du nouveau leader (et
			// modifier leur leader)
