...
/**
 * Created by moi on 12/05/14. Contient les actions basiques pouvant etre
 * utilisees dans les strategies.
 */
public class Actions implements IActions {
	...
	/**
	 * Appele par l'element. Permet a l'element ref1 (Personnage) de ramasser
	 * l'element ref2 (Potion), qui modifie les caracteristiques du personnage.
	 * 
	 * @param ref1
	 *            personnage ramassant la potion
	 * @param ref2
	 *            potion ramassee
	 * @param arene
	 *            arene
	...
	public void ramasser(int ref1, int ref2, IArene arene)
		...
		if (actionExecutee) {
			System.err
					.println("Une action a deja ete executee pendant ce tour !");
		} else {
			...
			// si les deux sont vivants (ils peuvent etre presents mais sans
			// vie)
			if (combattant.getElement().getVie() > 0
					&& potion.getElement().getVie() > 0) {
	 ...
	 * @throws RemoteException
	 */
	private void ramasserPotion(IConsole pot, IConsole per)
			throws RemoteException {
		...
		while (enumCaract.hasMoreElements()) {
			...
			// valeursPot.put(s, 0); //on vide toute la potion, meme si elle ne
			// correspond pas aux caract. du perso ?
	...
	/**
	 * Appele par le run de la console. Permet a l'attaquant (ref1) d'attaquer
	 * (executer une frappe) le defenseur (ref2). Les regles de combat
	 * s'appliquent (en fonction de la force, de la defense et de l'esquive).
	 * Les caracteristiques du defenseur sont mises a jour (pas d'impact sur
	 * attaquant) Les deux protagonistes ajoutent leur adversaire dans les
	 * elements deja vus.
	 * 
	 * @param ref1
	 *            attaquant
