...
public class DuelBasic implements IDuel {
	 ...
	 */
	public DuelBasic(IArene arene, IConsole att, IConsole def)
			throws RemoteException {
		this.arene = arene;
		this.attaquant = att;
		this.defenseur = def;
	...
	public boolean memeLeader(IConsole per1, IConsole per2)
		...
		return ((Personnage) per1.getElement()).getLeader() != -1
				&& ((Personnage) per1.getElement()).getLeader() == ((Personnage) per2
						.getElement()).getLeader();
	}
	...
	public void realiserCombat() throws RemoteException {
		...
		System.out.print("Duel entre " + attaquant.getRefRMI() + " et "
				+ defenseur.getRefRMI() + " : ");

		try {
			if (memeLeader(attaquant, defenseur)
					|| isLeader(attaquant, defenseur)) {
				// s'ils ont le meme leader ou que le defenseur est dans
				// l'equipe de l'attaquant, rien ne se passe
				System.out.println("Rien ne se passe");

			} else if (isLeader(defenseur, attaquant)) {
				if (attCharisme > defCharisme) {
					// coup d'etat
					System.out.println(attaquant.getRefRMI()
							+ " realise un coup d'etat contre "
							+ defenseur.getRefRMI());
				} else {
					// coup d'etat echoue
					System.out.println("Rien ne se passe");
	 ...
	 * 
	 * @param per
	 *            personnage qui a ajoute a son equipe (ou celle de son leader)
	 * @param eq
	 *            personnage a ajouter a l'equipe
	 */
	private void ajouterEquipe(IConsole per, IConsole eq)
			throws RemoteException {
		int leader = ((Personnage) per.getElement()).getLeader(); // ajouter au
																	// leader
																	// s'il y en
																	// a un
		...
		if (leader == -1) { // le personnage n'est pas dans une equipe
			...
			per.getElement().parler(
					"J'ajoute " + eq.getRefRMI() + " a mon equipe",
					per.getVueElement());
			System.out.println(per.getRefRMI() + " ajoute a son equipe "
					+ eq.getRefRMI());
			...
			lead.getElement().parler(
					"J'ajoute " + eq.getRefRMI() + " a mon equipe (par "
							+ per.getRefRMI() + ")", lead.getVueElement());
			System.out.println(leader + " ajoute a son equipe (par "
					+ per.getRefRMI() + ") " + eq.getRefRMI());
		}
	}
	...
	private void tuer(IConsole per, IConsole tue) throws RemoteException {
		per.getElement().parler("Je tue " + tue.getRefRMI(),
				per.getVueElement());
